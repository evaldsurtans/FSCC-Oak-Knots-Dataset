...
class LowPassFilters(nn.Module):
    """
...
class BandMask(nn.Module):
    ...
    def forward(self, wav):
        ...
        filters = LowPassFilters([mels[low], mels[high]]).to(wav.device)
...
class Shift(nn.Module):
    ...
    def __init__(self, shift=8192, same=False, is_train=True):
        """__init__.
        ...
        if self.shift > 0:
            if not self.is_train:
                ...
                offsets = torch.randint(
                    self.shift,
                    [1 if self.same else batch, 1, 1], device=wav.device)
                offsets = offsets.expand(-1, channels, -1)
                indexes = torch.arange(length, device=wav.device)
                wav = wav.gather(2, indexes + offsets)
                padding = torch.zeros((*wav.shape[:2], self.shift), device=wav.device)
...
class RevEcho(nn.Module):
    ...
    def forward(self, wav):
        ...
        reverb_wav = self._reverb(wav, initial, first_delay, rt60)
        wav += reverb_wav
